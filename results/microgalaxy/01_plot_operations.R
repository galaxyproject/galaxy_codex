...
df2 = df |>
    tidyr::separate_rows("EDAM operation low classes", sep = ",") |>
    setDT()

df2$`EDAM operation low classes` = df2$`EDAM operation low classes` |> str_squish()

df2 = df2[which(`EDAM operation low classes` != "")]

st = df2[, by = `EDAM operation low classes`, .N]
st = st[order(-N)]
st = st[seq_len(11)]

df2$cluster = ifelse(
    df2$`EDAM operation low classes` %in% st$`EDAM operation low classes`,
    df2$`EDAM operation low classes`, "Other operations"
)
...
df2$cluster = df2$cluster |> factor(levels = c(st$`EDAM operation low classes`, "Other operations"))
...
ggsave(
    plot = gr, filename = "Rplot_operations.png",
