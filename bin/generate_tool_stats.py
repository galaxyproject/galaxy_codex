...
def get_last_url_position(toot_id: str) -> str:
    ...
    url it returns the toot_id. So works for local and toolshed installed tools.
...
def add_tool_stats_to_tools(tool_stats_path: str, 
    ...
    community_tool_stats.to_csv(output_path, sep="\t", index=False) #store the merged file
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Merge tools list with tool stats')
    ...
    parser.add_argument('--label', 
                        '-la', 
                        help="Label for the added column e.g. usegalaxy.eu tool usage")
    ...
    community_tool_stats = add_tool_stats_to_tools(args.tool_stats,
                                           args.tools,
                                           args.stats_output,
    ...
    if args.wordcloud:
        get_wordcloud(community_tool_stats,
                    args.wordcloud_mask,
                    args.wc_output,
                    )
